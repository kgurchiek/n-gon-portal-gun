...
const b = {
    ...
    portalGunBullet(where = {
        ...
        const reflection = function() { // https://math.stackexchange.com/questions/13261/how-to-get-a-reflection-vector
            ...
            var angle = Math.atan2(nn.y, nn.x);

            if(input.down) {
              if (!level.hasCreatedBluePortal) {
            ...
            {
              if (!level.hasCreatedOrangePortal) {
                level.orangePortal.x = best.x - (75 * Math.cos(angle));
                level.orangePortal.y = best.y - (75 * Math.sin(angle));
                level.orangePortal.angle = angle;
              }
            ...
            portalGunPortal = level.portal({
              x: level.bluePortal.x,
              y: level.bluePortal.y
            }, level.bluePortal.angle + Math.PI, {
              x: level.orangePortal.x,
              y: level.orangePortal.y
            }, level.orangePortal.angle + Math.PI);
          
            //clear past portals
            level.customTopLayer = () => {
                level.oldCustomTopLayer();
                portalGunPortal[2].query();
                portalGunPortal[3].query();
                portalGunPortal[0].draw();
                portalGunPortal[1].draw();
                portalGunPortal[2].draw();
                portalGunPortal[3].draw();
            };
    ...
    portal(pos, velocity, type) {
        ...
        collide = Matter.Query.collides(this, map)
        bullet[me].do = function() {};
    },
    ...
    guns: [
        {
            name: "nail gun", // 0
            description: "<strong class='lore-text'>don't use this</strong class='lore-text'><br>(it totally isn't overpowered)",
            ammo: 0,
            ammoPack: Infinity,
            ...
            fireNormal() {
                b.nail({
                    ...
                    x: m.Vx / 2 + 30 * Math.cos(m.angle),
                    y: m.Vy / 2 + 30 * Math.sin(m.angle)
        ...
        {
            name: "portal gun", //12
            description: "creates <strong class='color-f'>por</strong class='color-f'><strong class='color-e'>tals</strong class='color-e'> when shot at a surface",
            ammo: 0,
            ammoPack: Infinity,
            have: false,
            charge: 0,
